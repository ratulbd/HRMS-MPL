...
async function getEmployees(sheets, SPREADSHEET_ID, EMPLOYEE_SHEET_NAME, HEADER_MAPPING, helpers) {
    ...
    try {
        ...
        return { statusCode: 500, body: JSON.stringify({ error: 'An internal server error occurred while fetching employees.', details: error.message })};
    }
}
...
async function updateStatus(sheets, SPREADSHEET_ID, EMPLOYEE_SHEET_NAME, HEADER_MAPPING, helpers, statusData) {
    ...
    for (const key in updates) { /* ... prepare updates ... */
    ...
    if (updates.hasOwnProperty('status') && updates.status !== 'Active') { /* ... implicit logic ... */
    ...
    } else if (updates.hasOwnProperty('salaryHeld') && !updates.hasOwnProperty('status')) { /* ... implicit logic ... */
    ...
    if (dataToUpdate.length > 0) { /* ... execute batch update ... */
...
async function getSubCenters(sheets, SPREADSHEET_ID, EMPLOYEE_SHEET_NAME, HEADER_MAPPING, helpers) {
    ...
    try {
        const headers = await helpers.getSheetHeaders(sheets, SPREADSHEET_ID, EMPLOYEE_SHEET_NAME);
        const subCenterHeader = HEADER_MAPPING.subCenter; const subCenterColIndex = headers.indexOf(subCenterHeader);
        if (subCenterColIndex === -1) return { statusCode: 200, body: JSON.stringify([]) };
        ...
        const uniqueSubCenters = [...new Set(rows.map(row => row[0]).filter(sc => sc && String(sc).trim() !== ''))].sort();
        return { statusCode: 200, body: JSON.stringify(uniqueSubCenters) };
         ...
         if (error.code === 400 && error.message.includes('Unable to parse range')) return { statusCode: 200, body: JSON.stringify([]) };
...
async function transferEmployee(sheets, SPREADSHEET_ID, EMPLOYEE_SHEET_NAME, HEADER_MAPPING, helpers, { employeeId, newSubCenter, reason, transferDate }) {
    ...
    try {
        ...
        const oldSubCenter = readResponse.data.valueRanges?.[1]?.values?.[0]?.[0] || 'N/A'; // This is the 'FROM' value
        console.log(`Fetched current values - Name: ${employeeName}, Old Sub Center: ${oldSubCenter}`);

        const dataToUpdate = [
            { // Update CURRENT Sub Center to the NEW one
            ...
            { // Update Last Subcenter (with the OLD subcenter)
                range: `${EMPLOYEE_SHEET_NAME}!${helpers.getColumnLetter(lastSubCenterColIndex)}${rowIndex}`,
                values: [[oldSubCenter]] // <-- SAVE THE OLD SUBCENTER HERE
            },
        ...
        const formattedTimestamp = helpers.formatDateForSheet(new Date());
        // Log still contains old (from) and new (to) subcenters
